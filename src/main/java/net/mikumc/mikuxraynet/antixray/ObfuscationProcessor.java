 ...
 *   <li>整块区块无任何改动时直接返回原字节数组，完全不触发重编码
 *       （codec 对未改动 section 会重编码出逐字节相同的结果，因此未改动部分等价于原样搬运）。</li>
 * </ul>
 *
...
public final class ObfuscationProcessor {
  ...
  public static ObfuscationProcessor create(ChunkCodec codec, BlockStateRegistry registry,
    ...
    return new ObfuscationProcessor(codec, registry::isOccluding, targets, replacementIds,
        cumulativeWeights, config.layerObfuscation());
  }
    ...
    try {
      for (int sectionIndex = 0; sectionIndex < chunk.getSectionCount(); sectionIndex++) {
        ...
        for (int index = 0; index < SECTION_VOLUME; index++) {
          ...
          if (!isFullyOccluded(chunk, baseY, index)) {
            continue;
          }
   ...
   */
  private boolean isFullyOccluded(Chunk chunk, int baseY, int index) {
    int x = index & 15;
    int z = index >> 4 & 15;
    int y = baseY | (index >> 8 & 15);

    return isOccluding(chunk, y + 1, x, z)
        && isOccluding(chunk, y - 1, x, z)
        && isOccluding(chunk, y, x + 1, z)
        && isOccluding(chunk, y, x - 1, z)
        && isOccluding(chunk, y, x, z + 1)
        && isOccluding(chunk, y, x, z - 1);
  }

  /** 区块内相对坐标处的方块是否遮挡；越出本区块（含缺失 section）一律视为未遮挡。 */
  private boolean isOccluding(Chunk chunk, int y, int x, int z) {
