 ...
 *
 * <p>动机：一次编解码里每个 section 都会新建调色板反查表（长度 = 唯一方块状态数，实测 32 KB 级）与位打包
 * long 数组，24 个 section 叠加就是每区块几百 KB 到 1 MB 级的分配量。这些数组只服务于「当前区块」，
 * 区块关闭后即可整批复用，因此这里按线程缓存：同一线程处理下一个区块时按同样顺序取回，稳态下几乎不再分配。
 *
 * <p>并发与生命周期约束：
 * <ul>
